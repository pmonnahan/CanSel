...
# 				   Order of individuals should be as in the singletons file (s_file).
    ...
    for chrom in chroms:
        ...
        for pop in pops:
            ...
            pdb.set_trace()
            pos = [x for x, j in zip(np.array(pos).tolist(), pol_key) if
                   j != 0]  # These 0 sites need to be removed from all arrays because they crash standardization
            flt = np.array([True if x != 0 else False for x in pol_key])
            pdb.set_trace()
