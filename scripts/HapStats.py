...
def format_results(stat, stat_name, chrom, pos, pop, polarization_key, ac="None", n_bins=20):
    ...
    flt2 = [False if isinf(x) else True for x in stat]
    pos = [y for x, y in zip(stat, pos) if not isinf(x)] #STOPPED HERE
    stat = [x for x in stat if not isinf(x)]
    if ac != "None":

        ac = ac.compress(flt, axis=0)
        ac = ac.compress(flt2, axis=0)
...
if __name__ == "__main__":
    ...
    # winsize = int(args.w) # Currently, no windowed approach for haplotype stats available in allel.
